...
Regarding the **serialization of `BigDecimal`**, JSON has no decimal type, only `number`. I'm aware that come JSON clients (such as javascript) parse these number into a floating point value, which can silently lose the precision that `BigDecimal` was meant to preserve. A solution to this would be to to serialize these values as **strings** and let the client parse them into its own decimal type. This constitutes a topic of discussion in online forums (string vs number) and it wasn't done here to mimic the upstream API which also doesn't serialize them as string.
